...
void DrumClient::requestRoomList() {
    if (!isConnected()) {
        qWarning() << "Impossible de demander la liste des salons sans connexion";
        return;
    }
...
void DrumClient::joinRoom(const QString& roomId, const QString& userId, const QString& userName, const QString& password) {
    if (!isConnected()) {
        qWarning() << "Impossible de rejoindre un salon sans connexion";
        return;
    }
    ...
    QByteArray message = Protocol::createMessage(MessageType::JOIN_ROOM, data);
    sendMessage(message);
}
...
void DrumClient::processMessage(const QByteArray& data) {
    ...
    if (!Protocol::parseMessage(data, type, messageData)) {
        qWarning() << "Impossible de parser le message reçu";
        return;
    }

    // Traiter les messages spécifiques au client
    switch (type) {
    case MessageType::ROOM_LIST_RESPONSE: {
        QJsonArray rooms = messageData["rooms"].toArray();
        emit roomListReceived(rooms);
        break;
    }

    case MessageType::ROOM_INFO: {
        emit roomStateReceived(messageData);
        break;
    }

    case MessageType::GRID_UPDATE: {
        GridCell cell = GridCell::fromJson(messageData);
        emit gridCellUpdated(cell);
        break;
    }

    default:
        // Pour tous les autres messages, émettre le signal générique
